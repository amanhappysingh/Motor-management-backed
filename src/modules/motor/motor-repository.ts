...
import {
  GetMotorDTO,
  GetMotorFilterDTO,
  MotorMoveToAvailableRequestDTO,
  MotorInRequestDTO,
  MotorMoveToOverhaulingRequestDTO,
  MotorMoveToTrialRequestDTO,
  MotorMoveToOutRequestDTO,
  MotorAnalyticsDTO,
  MotorAnalyticDTO,
  MotorMoveToFaultRequestDTO,
} from "./motor-dtos";
import { MotorOverhaulingAndTrialStatus, MotorStatus } from "../../core/types";
import env from "../../config/env";

export default class MotorRepository extends PostgresRepository {
  constructor() {
    super();
  }
  ...
  async motorIn(data: MotorInRequestDTO): Promise<void> {
    ...
    try {
      ...
      const result_motor: QueryResult = await client.query(
        `
        INSERT INTO motors (
          serial_no,
          motor_id,
      ...
      if (!result_motor.rowCount) throw new Error("Failed to motor in");

      const motorId: string = result_motor.rows[0].id;

      await client.query(
        `INSERT INTO motor_in (
          motor_id,
          location,
          in_at,
          in_by,
...
console.log("__params", params)
  ...
  return result.rows as GetMotorDTO[];
}
  ...
  async getMotor(motorId: string): Promise<GetMotorDTO> {
    ...
    const result: QueryResult = await this.pool.query(query, [motorId]);
    if(!result.rows) throw new Error("Motor not found")
    return result.rows[0] as GetMotorDTO;
  }

  async motorMoveToOverhauling(
    data: MotorMoveToOverhaulingRequestDTO
  ): Promise<void> {
    const client: PoolClient = await this.pool.connect();
    try {
      await client.query("BEGIN");
      const result: QueryResult = await client.query(
        ...
        [
          data.motor_id,
          MotorOverhaulingAndTrialStatus.Active,
          data.overhauled_started_by,
        ]
      );

      if (!result.rows) throw new Error("Failed to move in overhauling");

      await client.query(
        `UPDATE motors SET current_status = $1 WHERE id = $2`,
        [MotorStatus.Overhauling, data.motor_id]
      );
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK");
    ...
    try {
      ...
      const result: QueryResult = await client.query(
        ...
        [
          ...
          MotorOverhaulingAndTrialStatus.Completed,
          data.motor_id,
        ]
      );

      if (!result.rows) throw new Error("Failed to update status");
      ...
      await client.query(
        `UPDATE motors SET current_status = $1 WHERE id = $2`,
        [MotorStatus.Trial, data.motor_id]
      );
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK");
  ...
  async motorMoveToAvailable(
    data: MotorMoveToAvailableRequestDTO
  ): Promise<void> {
    const client: PoolClient = await this.pool.connect();
    try {
      await client.query("BEGIN");
      ...
      await client.query(
        `UPDATE motors SET current_status = $1 WHERE id = $2`,
        [MotorStatus.Available, data.motor_id]
      );
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK");
  ...
  async motorMoveToFault(data: MotorMoveToFaultRequestDTO): Promise<void> {
    ...
    try {
      ...
      const result: QueryResult = await client.query(`SELECT current_status FROM motors WHERE id = $1`, [data.motor_id])

      if(!result.rows) throw new Error("Motor not found")

      const currentStatus: string = result.rows[0].current_status
      ...
      await client.query(
        `UPDATE motors SET current_status = $1 WHERE id = $2`,
        [MotorStatus.Fault, data.motor_id]
      );
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK");
  ...
  async motorAnalytics(userId: string): Promise<MotorAnalyticsDTO> {
    const defaultWeek: Record<string, MotorAnalyticDTO> = {
      monday: {
        in: 0,
        overhauling: 0,
        out: 0,
    ...
    const defaultYear: Record<string, MotorAnalyticDTO> = monthKeys.reduce(
      (acc, month) => {
        acc[month] = {
          in: 0,
          overhauling: 0,
      ...
      {} as Record<string, MotorAnalyticDTO>
    );

    // Overall
    const overallResult: QueryResult = await this.pool.query(`
    SELECT
  ...
  `);
    ...
    // Today
    const todayResult: QueryResult = await this.pool.query(`
    SELECT
      COUNT(*) FILTER (WHERE current_status='In') AS in_count,
      COUNT(*) FILTER (WHERE current_status='Overhauling') AS overhauling_count,
      COUNT(*) FILTER (WHERE current_status='Out') AS out_count,
  ...
  `);
    ...
    // Last week
    const currentWeekResult: QueryResult = await this.pool.query(`
    SELECT
  ...
  `);
    ...
    currentWeekResult.rows.forEach((row: any) => {
    ...
    // Last year
    const currentYearResult: QueryResult = await this.pool.query(`
    SELECT
  ...
  `);
    ...
    currentYearResult.rows.forEach((row: any) => {
